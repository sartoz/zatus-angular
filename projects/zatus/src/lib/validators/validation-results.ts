...
export class ValidationResults<T> {
    ...
    public static createWithPreviousFailures = <T>(
        previousFailures?: T[] | ValidationResults<T>,
        allowDuplicateFailures: boolean = true,
    ): ValidationResults<T> => {
        const validationResult = new ValidationResults<T>(
            allowDuplicateFailures,
        );
        return previousFailures instanceof ValidationFailure
            ? validationResult.fillWithPreviousFailuresFromValidationResults(
                  previousFailures as any,
              )
            : validationResult.fillWithPreviousFailuresFromGenericArray(
                  previousFailures as any,
              );
    };

    protected fillWithPreviousFailuresFromValidationResults = (
        previousFailures: ValidationResults<T>,
    ): ValidationResults<T> => {
    ...
    protected fillWithPreviousFailuresFromGenericArray = (
        previousFailures: T[],
    ): ValidationResults<T> => this.addFailures(previousFailures);
    ...
    protected throwErrorIfNotAllowDuplicateFailure = (
        failures: T | T[],
    ): void => {
